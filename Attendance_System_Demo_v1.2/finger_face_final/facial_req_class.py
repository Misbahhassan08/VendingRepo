...
from R307_arduino_RPI import R307
...
class faceRecognition_thread(QtCore.QThread):
    ...
    Finger_signalFlow = pyqtSignal(str,str,name='Finger_signal')
    def __init__(self):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.scan_id:
                self.finger_Id = self.r307.get_finger_id()
                if self.finger_Id == 3:
                    self.Finger_signalFlow.emit(str(self.finger_Id),'Finger')
                    print("Pickle file loading Done ")
                    self.data = pickle.loads(open(f"{self.finger_Id}/{self.model_Folder}/{self.encodingsP}", "rb").read())
                    self.scan_id = False
