...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def init_connections(self):
        ...
        self.Face_Recognition_thread.Rfid_signalFlow.connect(self.rfid_data)
    ...
    def rfid_data(self,ID,Name):
        self.id = ID
        print(f"rfid py signal working {self.id}")
        self.name = Name
        self.chek_loop()
        pass
    def face_Name(self,Face_name):
        self.face_name = Face_name
        print(f"face_name py signal is working with {self.face_name}")
        self.chek_loop()
        self.face_name = ""
        pass
    def chek_loop(self):
        if self.id == "417650910869":
           self.label.setText(f"Your ID = {self.id} Please Show Your Face..")
           if self.face_name == self.id:
               self.label.setText(f"Present With ID : {self.face_name} Present...")
               self.id = ""
               self.face_name = ""
        pass
'''
